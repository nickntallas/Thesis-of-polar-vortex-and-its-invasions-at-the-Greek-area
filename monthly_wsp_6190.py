...
# θέλουμε να ελέγξουμε ξεχωριστά κάθε μήνα της τριακονταετίας 1961-1990
for year in range(1961, 1990):
    for month in range(1, 13):
        df_1961_1990 = ds.sel(time=slice(str(year) + '-' + str(month).zfill(2), str(year) + '-'
                                         + str(month).zfill(2))).to_dataframe()

        # επεξεργασία των μετρήσεων
        df_1961_1990 = df_1961_1990.reset_index()
        df_1961_1990 = df_1961_1990[(df_1961_1990['longitude'] % 5 == 0) & (df_1961_1990['latitude'] % 5 == 0)]
        df_1961_1990.index = df_1961_1990['time']
        df_1961_1990 = df_1961_1990.drop(columns=['time'])
        df_1961_1990.columns = ['Lon', 'Lat', 'P(hPa)', 'U(m/s)', 'V(m/s)']

        # υπολογισμός της ταχύτητας του ανέμου από τις συνιστώσες του
        df_1961_1990['WS(m/s)'] = np.sqrt(df_1961_1990['U(m/s)'].pow(2) + df_1961_1990['V(m/s)'].pow(2))

        # Εντοπισμός μοναδικών τιμών για το γεωγραφικό μήκος στη στήλη 'Lon'
        unique_lon_values = df_1961_1990['Lon'].unique()
        ...
        for lon_value in unique_lon_values:
            ...
            df_lon = df_1961_1990.loc[df_1961_1990['Lon'] == lon_value].copy()
...
with open('matching_files_1961_1990.txt', 'w') as f:
